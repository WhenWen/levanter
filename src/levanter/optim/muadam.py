...
@OptimizerConfig.register_subclass("muadam")
@dataclass
class MuAdamConfig(OptimizerConfig):
    """
    MuAdam optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    muadam_to_adam_lr: float = 0.18  # Scaling factor between AdamW and MuAdam learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            ...
            def muadam_transform():
                components = []
                components.append(scale_with_muadam(self.momentum, self.beta2, self.nesterov, self.backend_steps))
...
class ScaleByMuAdamState(NamedTuple):
...
def scale_with_muadam(momentum=0.95, b2 = 0.95, nesterov=True, steps=5, eps = 1e-7):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        nu = otu.tree_zeros_like(params)  # Second moment
        return ScaleByMuAdamState(count=jnp.zeros([], jnp.int32), momentum_buffer=momentum_buffer, nu = nu )
    ...
    def update_fn(updates, state, params=None):
        ...
        return updates, ScaleByMuAdamState(count = state.count + 1, momentum_buffer=buf, nu = nu)
