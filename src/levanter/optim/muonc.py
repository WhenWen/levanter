...
@OptimizerConfig.register_subclass("muonC")
@dataclass
class MuonCConfig(OptimizerConfig):
    """
    MuonC optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    lr: float = 0.02
    muonC_to_adam_lr: float = 0.18  # Scaling factor between AdamW and MuonC learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.muonC_to_adam_lr

            def muonC_transform():
                components = []
                # MuonC seems incompatible with gradient clipping, need to investigate
                # if self.max_grad_norm:
                #     components.append(optax.clip_by_global_norm(self.max_grad_norm))
                components.append(scale_with_muonC(self.momentum, self.nesterov, self.backend_steps))
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'muonC' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                # muonC for linear layers
                return dataclasses.replace(param, weight="muonC", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByMuonCState(NamedTuple):
    """State for the Mars algorithm."""

    momentum_buffer: optax.Updates


def scale_with_muonC(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByMuonCState(momentum_buffer=momentum_buffer)
    ...
    def update_fn(updates, state, params=None):
        ...
        muon_updates = map_flattened_linear_layers(transform_linear_layer, muon_updates)
        cautious_m = jax.tree.map(
            lambda u, g: None if u is None else (g * u) > 0, 
            muon_updates,
            updates,
            is_leaf=lambda x: x is None
        )
        cautious_update = jax.tree.map(
            lambda u, m: None if u is None else u * m / (m.mean() + 1e-7),
            muon_updates,
            cautious_m,
            is_leaf=lambda x: x is None
        ) 
        return cautious_update, ScaleByMuonCState(momentum_buffer=buf)
