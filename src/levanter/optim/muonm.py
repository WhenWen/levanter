...
@OptimizerConfig.register_subclass("muonM")
@dataclass
class MuonMConfig(OptimizerConfig):
    """
    MuonM optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    lr: float = 0.02
    muonM_to_adam_lr: float = 0.18  # Scaling factor between AdamW and MuonM learning rates
    momentum: float = 0.95
    momentum2: float = 0.5
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.muonM_to_adam_lr

            def muonM_transform():
                components = []
                # MuonM seems incompatible with gradient clipping, need to investigate
                # if self.max_grad_norm:
                #     components.append(optax.clip_by_global_norm(self.max_grad_norm))
                components.append(scale_with_muonM(self.momentum, self.momentum2, self.nesterov, self.backend_steps))
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'muonM' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                # muonM for linear layers
                return dataclasses.replace(param, weight="muonM", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByMuonMState(NamedTuple):
...
def scale_with_muonM(momentum=0.95, momentum2 = 0.5, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        outer_momentum_buffer = otu.tree_zeros_like(params)  # Second moment
        return ScaleByMuonMState(momentum_buffer=momentum_buffer, outer_momentum_buffer = outer_momentum_buffer, count=jnp.zeros([], jnp.int32))

    def update_fn(updates, state, params=None):
        buf = state.momentum_buffer
        out_buf = state.outer_momentum_buffer
        buf = jax.tree.map(
        ...
        out_buf = jax.tree.map(
            lambda m, g: None if g is None else momentum2 * m + (1 - momentum2) * g,
            out_buf,
            updates,
            is_leaf=lambda x: x is None,
        )
        updates = otu.tree_bias_correction(out_buf, momentum2, state.count + 1)

        return updates, ScaleByMuonMState(momentum_buffer=buf, outer_momentum_buffer=out_buf, count=state.count + 1)
