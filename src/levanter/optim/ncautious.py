...
@OptimizerConfig.register_subclass("ncautious")
@dataclass
class NCautiousConfig(OptimizerConfig):
    beta1: float = 0.95
    ...
    def build(self, num_train_steps):
        ...
        def _optimizer(learning_rate):
            ...
            components.append(scale_by_ncautious(self.beta1, self.beta2, self.epsilon))
...
class ScaleByNCautiousState(NamedTuple):
...
def scale_by_ncautious(
  ...
  def init_fn(params):
    ...
    return ScaleByNCautiousState(count=jnp.zeros([], jnp.int32), mu=mu, nu=nu)
  ...
  def update_fn(updates, state, params=None):        
    ...
    ncautious_m = jax.tree.map(
       lambda u, g: None if u is None else (g * u) > 0, 
       adam_updates,
       updates,
       is_leaf=lambda x: x is None
    )
    ncautious_update = jax.tree.map(
    ...
    return ncautious_update, ScaleByNCautiousState(count=count_inc, mu=mu, nu=nu)
