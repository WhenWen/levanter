...
@OptimizerConfig.register_subclass("marss")
@dataclass
class MarsSimpConfig(OptimizerConfig):
...
class ScaleByMarsSimpState(NamedTuple):
...
def scale_by_mars(
  ...
  def init_fn(params):
    ...
    return ScaleByMarsSimpState(count=jnp.zeros([], jnp.int32), mu=mu, nu=nu, mog = mog)
  ...
  def update_fn(updates, state, params=None):        
    ...
    return adam_updates, ScaleByMarsSimpState(count=count_inc, mu=mu, nu=nu, mog = updates)
