...
@OptimizerConfig.register_subclass("noum3")
@dataclass
class Noum3Config(OptimizerConfig):
    """
    Noum3 optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    noum3_to_adam_lr: float = 1.0 # Scaling factor between AdamW and Noum3 learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.noum3_to_adam_lr

            def noum3_transform():
                components = []
                components.append(scale_with_noum3(self.momentum, self.nesterov, self.backend_steps))
            ...
            def noum3_1d_transform():
                components = []
                components.append(scale_with_noum3_1d(self.momentum, False, self.backend_steps))
                if self.weight_decay > 0:
                    components.append(optax.add_decayed_weights(self.weight_decay, self.build_weight_decay_mask()))
            ...
            transformations = {
                "noum3": noum3_transform(),
                "adamw": noum3_1d_transform(),
            }
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'noum3' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                return dataclasses.replace(param, weight="noum3", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByNoum3State(NamedTuple):
    """State for the Mars algorithm."""

    momentum_buffer: optax.Updates


def scale_with_noum3(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByNoum3State(momentum_buffer=momentum_buffer)
    ...
    def update_fn(updates, state, params=None):
        ...
        return updates, ScaleByNoum3State(momentum_buffer=buf)

    return optax.GradientTransformation(init_fn, update_fn)

def scale_with_noum3_1d(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByNoum3State(momentum_buffer=momentum_buffer)
