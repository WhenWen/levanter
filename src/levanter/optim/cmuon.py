...
@OptimizerConfig.register_subclass("cmuon")
@dataclass
class CMuonConfig(OptimizerConfig):
    """
    CMuon optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    cmuon_to_adam_lr: float = 0.18  # Scaling factor between AdamW and CMuon learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.cmuon_to_adam_lr

            def cmuon_transform():
                components = []
                if self.max_grad_norm:
                    components.append(optax.clip_by_global_norm(self.max_grad_norm))
                components.append(scale_with_cmuon(self.momentum, self.nesterov, self.backend_steps))
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'cmuon' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                # cmuon for linear layers
                return dataclasses.replace(param, weight="cmuon", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByCMuonState(NamedTuple):
    """State for the Mars algorithm."""
    count: jaxtyping.Array  # shape=(), dtype=jnp.int32.
    momentum_buffer: optax.Updates


def scale_with_cmuon(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByCMuonState(count=jnp.zeros([], jnp.int32), momentum_buffer=momentum_buffer, )
    ...
    def update_fn(updates, state, params=None):
        ...
        if nesterov:
            ...
            updates = bias_correction(updates, momentum, state.count + 1)
        else:
            updates = buf
            updates = bias_correction(updates, momentum, state.count + 1)
        ...
        return updates, ScaleByCMuonState(count = state.count + 1, momentum_buffer=buf)
