...
@OptimizerConfig.register_subclass("amuon")
@dataclass
class AMuonConfig(OptimizerConfig):
    """
    AMuon optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    lr: float = 0.02
    amuon_to_adam_lr: float = 0.18  # Scaling factor between AdamW and AMuon learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.amuon_to_adam_lr

            def amuon_transform():
                components = []
                if self.max_grad_norm:
                    components.append(optax.clip_by_global_norm(self.max_grad_norm))
                components.append(scale_with_amuon(self.momentum, self.nesterov, self.backend_steps))
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'amuon' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                # amuon for linear layers
                return dataclasses.replace(param, weight="amuon", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByAMuonState(NamedTuple):
    """State for the Mars algorithm."""

    momentum_buffer: optax.Updates


def scale_with_amuon(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByAMuonState(momentum_buffer=momentum_buffer)
    ...
    def update_fn(updates, state, params=None):
        ...
        return updates, ScaleByAMuonState(momentum_buffer=buf)
