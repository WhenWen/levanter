...
@OptimizerConfig.register_subclass("noum")
@dataclass
class NoumConfig(OptimizerConfig):
    """
    Noum optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    noum_to_adam_lr: float = 0.18  # Scaling factor between AdamW and Noum learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.noum_to_adam_lr

            def noum_transform():
                components = []
                components.append(scale_with_noum(self.momentum, self.nesterov, self.backend_steps))
            ...
            def noum_1d_transform():
                components = []
                if self.max_grad_norm:
                    components.append(optax.clip_by_global_norm(self.max_grad_norm))
                components.append(scale_with_noum_1d(self.momentum, self.nesterov, self.backend_steps))
                if self.weight_decay > 0:
                    components.append(optax.add_decayed_weights(self.weight_decay, self.build_weight_decay_mask()))
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'noum' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                return dataclasses.replace(param, weight="noum", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByNoumState(NamedTuple):
    """State for the Mars algorithm."""

    momentum_buffer: optax.Updates


def scale_with_noum(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByNoumState(momentum_buffer=momentum_buffer)
    ...
    def update_fn(updates, state, params=None):
        ...
        return updates, ScaleByNoumState(momentum_buffer=buf)

    return optax.GradientTransformation(init_fn, update_fn)

def scale_with_noum_1d(momentum=0.95, nesterov=True, steps=5):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        return ScaleByNoumState(momentum_buffer=momentum_buffer)
    ...
    def update_fn(updates, state, params=None):
        ...
        updates = jax.tree_map(
            lambda u: None if u is None else u / (jnp.linalg.norm(u, axis=-1, keepdims=True) + 1e-8),
            updates,
            is_leaf=lambda x: x is None
        )
