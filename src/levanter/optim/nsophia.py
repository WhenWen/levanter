...
class ScaleByNSophiaState(NamedTuple):
    """State for NSophia and similar."""
...
# @runtime_checkable
# class NSophiaGObjective(typing.Protocol):
#     """
#     Class for objective functions that can be used with NSophia-G
#
#     NSophia-G is a second order optimizer that uses the Gauss-Newton-Bartlett approximation to the Hessian
...
#         """
#         Just a convenience method for invoking the objective for "normal" training w/o nsophia-g
#         """
...
#
#         By default, we just return 1, and you can just pull the term into the hyperparams of NSophia if you want.
#
#         Returns:
...
#
#
#     def apply_partial(self, *args, **kwargs) -> "NSophiaGObjective":
#         """
...
#
#
# class PartialNSophiaG(NSophiaGObjective):
#     def __init__(self, objective: NSophiaGObjective, *args, **kwargs):
...
#
#    def apply_partial(self, *args, **kwargs) -> NSophiaGObjective:
#        return PartialNSophiaG(self.objective, *self.args, *args, **self.kwargs, **kwargs)


@dataclass
class BaseNSophiaConfig(HessianOptConfig):
    """Base class for nsophia variants. Doesn't implement the state update"""
    ...
    def build(self, num_train_steps: int):
        def _optimizer(learning_rate, gamma) -> optax.GradientTransformation:
            ...
            components.append(
                _nsophia_gradient_transform(
                    nsophia_hess_fn=self.compute_hessian,
...
# @OptimizerConfig.register_subclass("nsophia-g")
# @dataclass
# class NSophiaGConfig(BaseNSophiaConfig):
...
@OptimizerConfig.register_subclass("nsophia-h")
@dataclass
class NSophiaHConfig(BaseNSophiaConfig):
...
def nsophia_h(
    ...
    *,
    ...
    """NSophia-H: https://arxiv.org/pdf/2305.14342.pdf Algorithm 1&3"""
    components = []

    components.append(scale_by_nsophia_h(b1, b2, eps, gamma, clip_threshold, update_interval, key=key))
...
def scale_by_nsophia_h(
    ...
    *,
    ...
    return _nsophia_gradient_transform(
        nsophia_hess_fn=stochastic_hessian_diagonal,
...
def nsophia_g(
    ...
    *,
    ...
    """NSophia-G: https://arxiv.org/pdf/2305.14342.pdf Algorithm 2&3"""
    components = []

    components.append(scale_by_nsophia_g(b1, b2, eps, gamma, clip_threshold, update_interval, key=key))
...
def scale_by_nsophia_g(
    ...
    *,
    ...
    return _nsophia_gradient_transform(
        nsophia_hess_fn=stochastic_diag_gauss_newton,
        update_interval=update_interval,
        b1=b1,
        b2=b2,
        eps=eps,
...
def _nsophia_gradient_transform(
    nsophia_hess_fn,
    update_interval: int,
    b1: float,
    b2: float,
    ...
    def update_fn(updates, state, params=None, *, obj_fn, **kwargs):
        ...
        mu_hat = jax.tree.map(
            lambda m, g:  None if m is None else (1 - b1) * g + b1 * m,
            mu,
            updates,
        )
        mu_hat = bias_correction(mu_hat, b1, state.count + 1)
        h_hat = state.h
        ...
        # with nsophia-g the max(h, 0) is not needed but no harm
    ...
    def update_hessian(state, params, *, obj_fn, **kwargs):
        def _do_update():
            ...
            new_hess = nsophia_hess_fn(obj_fn, params, hess_key=key, **kwargs)
...
def stochastic_diag_gauss_newton(fn, model, *args, hess_key: PRNGKey, **kwargs):
    ...
    Args:
        fn (NSophiaGObjective): objective function
        model: model whose Hessian to compute
        hess_key: key for sampling
        *args, **kwargs: passed to fn's logits
    """
    raise NotImplementedError("This is not implemented yet")
    # if not isinstance(fn, NSophiaGObjective):
    #     raise ValueError("objective must be a NSophiaGObjective")
...
def stochastic_hessian_diagonal(fn, model, *args, hess_key: PRNGKey, **kwargs):
    ...
    Args:
        ...
        clip_norm: optional global clipping norm for the Hessian diagonal
    """
