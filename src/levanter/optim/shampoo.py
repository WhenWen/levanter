...
class ShampooConfig(OptimizerConfig):
  # Basic learning rate and block-related hyperparameters.
  block_size: int = 128

  # Exponential averaging for moments.
  beta1: float = 0.9
  beta2: float = 0.999
  ...
  # When to start using full-matrix Shampoo updates.
  start_preconditioning_step: int = 5

  # Frequency (in steps) at which to recompute the preconditioners.
  ...
  # Grafting type used to “fix” the scale.
  graft_type: GraftingType = GraftingType.SGD

  # Whether to use Nesterov momentum.
  nesterov: bool = True

  # Optionally override the inverse power exponent.
  exponent_override: int = 0

  # For distributed training, the name of the batch axis.
  batch_axis_name: Optional[str] = None

  # PartitionSpecs for statistics and preconditioners (for pjit/sharded mode).
  statistics_partition_spec: Optional[Any] = None
  preconditioner_partition_spec: Optional[Any] = None
  num_devices_for_pjit: Optional[int] = None

  # Whether to shard the optimizer states.
  shard_optimizer_states: bool = False

  # Experimental memory reduction.
  best_effort_memory_usage_reduction: bool = False

  # Controls inverse pth-root failure behavior.
  inverse_failure_threshold: float = 0.1

  # Whether to use moving average for momentum.
  moving_average_for_momentum: bool = False

  # Skip preconditioning for very large dimensions.
  skip_preconditioning_dim_size_gt: int = 4096

  # Optionally clip gradients by the norm after scaling.
  clip_by_scaled_gradient_norm: Optional[float] = None

  # Numerical precision settings.
  precision: Any = jax.lax.Precision.HIGHEST
  tensordot_precision: Optional[Any] = None
  relative_matrix_epsilon: bool = True
  merge_small_dims_block_size: int = 4096

  # Options for LOBPCG-based eigen-decomposition.
  lobpcg_topk_precondition: int = 0
  lobpcg_max_iter: int = 0

  # Preconditioner type: ALL, INPUT, or OUTPUT.
  precondtioner_type: PreconditionerType = PreconditionerType.ALL

  # Option to supply a custom preconditioner.
  custom_preconditioner: bool = False

  # Skip preconditioning for parameters with low rank.
  skip_preconditioning_rank_lt: int = 1

  # Whether to decouple the learning rate from the preconditioned update.
  decoupled_learning_rate: bool = True

  # Whether to decouple weight decay from the scaled update.
  decoupled_weight_decay: bool = False

  # Whether to generate training metrics.
  generate_training_metrics: bool = True

  # Whether to reuse preconditioners between steps.
  reuse_preconditioner: bool = False

  # Optionally use eigen-decomposition (eigh) for the inverse pth-root.
  eigh: bool = False

  # Use AdaGrad grafting for parameters whose shape contains these dimensions.
  adagrad_dims: Tuple = ()
  adagrad_learning_rate: float = 1.0
  ...
  def build(self, num_train_steps):
      ...
      def optimizer(learning_rate):
          ...
          components.append(distributed_shampoo(
              ...
              statistics_partition_spec=self.statistics_partition_spec,
              preconditioner_partition_spec=self.preconditioner_partition_spec,
              ...
              tensordot_precision=self.tensordot_precision,
              ...
              precondtioner_type=self.precondtioner_type,
              custom_preconditioner=self.custom_preconditioner,
              skip_preconditioning_rank_lt=self.skip_preconditioning_rank_lt,
              decoupled_learning_rate=self.decoupled_learning_rate,
              decoupled_weight_decay=self.decoupled_weight_decay,
              generate_training_metrics=self.generate_training_metrics,
              reuse_preconditioner=self.reuse_preconditioner,
              eigh=self.eigh,
              adagrad_dims=self.adagrad_dims,
              adagrad_learning_rate=self.adagrad_learning_rate,
          ))
