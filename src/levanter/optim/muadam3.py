...
@OptimizerConfig.register_subclass("muadam3")
@dataclass
class MuAdam3Config(OptimizerConfig):
    """
    MuAdam3 optimizer configuration: Momentum Orthogonalized by Newton-Schulz.
    """

    muadam3_to_adam_lr: float = 0.18  # Scaling factor between AdamW and MuAdam3 learning rates
    momentum: float = 0.95
    nesterov: bool = True
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate):
            adam_lr = learning_rate * self.muadam3_to_adam_lr

            def muadam3_transform():
                components = []
                components.append(scale_with_muadam3(self.momentum, self.beta2, self.nesterov, self.backend_steps))
    ...
    def create_mask(self, params):
        """
        Creates a mask that labels parameters as 'muadam3' or 'adamw' based on their
        ...
        def mask_fn(param, path):
            ...
            if "Embedding" in path_str or "lm_head" in path_str:
                ...
                # muadam3 for linear layers
                return dataclasses.replace(param, weight="muadam3", bias="adamw" if param.bias is not None else None)
            else:
                return "adamw"
...
class ScaleByMuAdam3State(NamedTuple):
...
def scale_with_muadam3(momentum=0.95, b2 = 0.95, nesterov=True, steps=5, eps = 1e-7):
    def init_fn(params):
        momentum_buffer = otu.tree_zeros_like(params)  # First moment
        nu = otu.tree_zeros_like(params)  # Second moment
        return ScaleByMuAdam3State(count=jnp.zeros([], jnp.int32), momentum_buffer=momentum_buffer, nu = nu )
    ...
    def update_fn(updates, state, params=None):
        ...
        mu_updates = map_flattened_linear_layers(transform_linear_layer, adam_updates)

        updates = jax.tree.map(lambda m, v: None if m is None else m * 0.5 + v * 0.5,
            buf,
            nu_hat,
            is_leaf=lambda x: x is None,
        )

        return updates, ScaleByMuAdam3State(count = state.count + 1, momentum_buffer=buf, nu = nu)
