...
while true; do
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$STATE" != "READY" ]; then
      ...
      # gcloud compute tpus tpu-vm ssh  --zone $ZONE  $VM_NAME --command="source venv*/bin/activate && pip3 uninstall draccus && pip3 install -U draccus" --worker=all
      gcloud compute tpus tpu-vm scp src/levanter/optim/soap_old.py  $VM_NAME:levanter/src/levanter/optim/soap.py --zone $ZONE --worker=all
