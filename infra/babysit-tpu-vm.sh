...
while true; do
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$STATE" != "READY" ]; then
      ...
      gcloud compute tpus tpu-vm scp --recurse src/levanter/optim/soap.py $VM_NAME:levanter/src/levanter/optim/soap.py --zone $ZONE --worker=all
      ...
      # gcloud compute tpus tpu-vm scp src/levanter/optim/util.py  $VM_NAME:levanter/src/levanter/optim/util.py --zone $ZONE --worker=all
